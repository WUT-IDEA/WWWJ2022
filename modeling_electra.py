...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        # print(discriminator_hidden_states[0].shape)#shape：(batchsize, max_len, 768)max_len不一定是最大，如果超过则截断
        sequence_output = discriminator_hidden_states[0]#这里0就是hidden_states
...
class ElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        start_logits = start_logits.squeeze(-1)#删掉最后一维向量
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_positions.clamp_(0, ignored_index)#把开始结束位置压缩在“0-长度”内
...
class ElectraClassificationStartEnd(nn.Module):
    """Head for sentence-level classification tasks."""
    ...
    def forward(self, features, start, end, tz, **kwargs):
        print(start,end)
        for i in range(2):
            if start[i]<end[i]+1:
                pooled_output = self.sequence_summary(features[i, start[i]:end[i]+1, :])
                tz[i] = pooled_output
                # tz[i] = torch.mean(features[i, start[i]:end[i]+1, :], 0, keepdim=False, out=None)
            else:
                tz[i] = features[i, 0, :]
        x = tz 
        x = self.dropout(x)
...
class ElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.electra = ElectraModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size, config.num_labels)
        self.classifier = ElectraClassificationStartEnd(config)
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_loss = loss_fct(start_logits, start_positions)
            end_loss = loss_fct(end_logits, end_positions)
            loss1 = (start_loss + end_loss) / 2

        start = start_logits.argsort()
        end = end_logits.argsort()
        list_s = []
        list_e = []
        for i in start:
            list_s.append(i[-1])
        for j in end:
            list_e.append(j[-1])
        start = torch.tensor(list_s) 
        end = torch.tensor(list_e)#shape = (1, batchsize)

        device = input_ids.device if input_ids is not None else inputs_embeds.device
        tz = torch.zeros((2, 768), device=device)
        logits2 = self.classifier(sequence_output,start,end,tz)
        loss2 = None
        if labels is not None:
            loss_fct2 = CrossEntropyLoss()
            loss2 = loss_fct2(logits2.view(-1, self.num_labels), labels.view(-1))
